...
time_trigger::OneTimeTrigger::OneTimeTrigger(
    const gpio::IGpioPtr &gpio, const logger::SysLoggerPtr &sysLogger)
    : m_gpio(gpio), m_sysLogger(sysLogger) {}

time_trigger::OneTimeTrigger::~OneTimeTrigger() { m_stopThread = true; }
...
void OneTimeTrigger::threadFn(size_t duration) {
  ...
  for (size_t durationCounter = 0;
       (false == m_stopThread) && (durationCounter < duration);
       ++durationCounter) {
    sleep(1);
  }
