...
static const size_t ON_TIME_DURATION = 10 * MIN_TO_SEC;      // call upcate intervall for thread
static const size_t ON_OFFSET_NIGHT_TARIF = 15 * MIN_TO_SEC; // call upcate intervall for thread

NightAir::NightAir(const gpio::IGpioPtr& gpio, const logger::SysLoggerPtr& sysLogger)
    : m_gpio(gpio)
    , m_sysLogger(sysLogger)
{
    ...
    addTimer(8 * HOUR_TO_SEC + ON_OFFSET_NIGHT_TARIF);
}

void NightAir::addTimer(size_t onTime)
{
    time_trigger::TimeTriggerPtr timer = std::make_shared<time_trigger::TimeTrigger>(
